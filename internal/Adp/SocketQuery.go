...
func (d AdpRecaller) ImplementRecall(data C.CData) C.CData {
	...
	case iAction == C.ADD_CUSTOMER:
		...
		tmp, ok := raw.(string)
		if !ok {
			sError = fmt.Sprintf("欄位 id 類型錯誤：%T", raw)
			break
		}
		if tmp == "" {
			sError = "欄位 id 為空"
		...
		sType := strings.Split(tmp, "-")
		if len(sType) < 1 {
			sError = "資料異常，無法辨識客群"
			break
		}

		sId := sType[0] + "-" + GetNewCusId()
